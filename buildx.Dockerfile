...
WORKDIR /go/src/github.com/traefik/hub-agent
...
RUN OUTPUT="dist/$TARGETPLATFORM/hub-agent" GOOS=${TARGETOS} GOARCH=${TARGETARCH} GOARM=${TARGETVARIANT/v/} make build
...
ARG TARGETPLATFORM
COPY --from=gobuild /go/src/github.com/traefik/hub-agent/dist/$TARGETPLATFORM/hub-agent /
